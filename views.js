          ...
          <div className="view-fade-in">
            ...
            {wizardStep === 1 && (
              <div className="bg-white rounded-xl shadow-lg p-3">
                {/* City Selector - prominent city name + dropdown */}
                <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '8px', marginBottom: '8px' }}>
                  <span style={{ fontSize: '18px', fontWeight: 'bold', color: '#e11d48' }}>{window.BKK.selectedCity?.icon} {tLabel(window.BKK.selectedCity)}</span>
                  {Object.values(window.BKK.cities).filter(c => c.active !== false).length > 1 && (
                    <select
                      value={selectedCityId}
                      onChange={(e) => switchCity(e.target.value)}
                      style={{ padding: '3px 6px', borderRadius: '8px', border: '1px solid #e5e7eb', fontSize: '10px', color: '#6b7280', background: 'white', cursor: 'pointer' }}
                    >
        ...
        {currentView === 'form' && (!wizardMode || wizardStep === 3) && (
          <div className="view-fade-in bg-white rounded-xl shadow-lg p-3 space-y-3">
            ...
            <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '8px' }}>
              <span style={{ fontSize: '16px', fontWeight: 'bold', color: '#e11d48' }}>{window.BKK.selectedCity?.icon} {tLabel(window.BKK.selectedCity)}</span>
              {Object.values(window.BKK.cities).filter(c => c.active !== false).length > 1 && (
                <select
                  value={selectedCityId}
                  onChange={(e) => switchCity(e.target.value)}
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px', flexWrap: 'wrap' }}>
                  ...
                  >
                    {Object.values(window.BKK.cities).map(city => (
                      <option key={city.id} value={city.id}>{city.icon} {tLabel(city)} ({city.areas?.length || 0} {t('general.areas')})</option>
                    ))}
                  </select>
                  {isUnlocked && (
                ...
                {(() => {
                  ...
                  return (
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px', padding: '6px 10px', background: isActive ? '#ecfdf5' : '#fef2f2', borderRadius: '8px', border: `1px solid ${isActive ? '#a7f3d0' : '#fecaca'}`, flexWrap: 'wrap' }}>
                      ...
                      <span style={{ fontSize: '10px', color: '#6b7280' }}>{city.areas?.length || 0} {t('general.areas')} · {city.interests?.length || 0} {t('nav.interests')}</span>
                      {isUnlocked && (
                        <>
                          <button onClick={() => {
